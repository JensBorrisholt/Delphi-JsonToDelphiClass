...
begin
  ...
  try
    Assert.AreEqual(Ord(jvkString), Ord(DataType.JsonKind));
    Assert.AreEqual(Ord(svkGuid), Ord(DataType.SemanticKind));
...
begin
  ...
  try
    ...
    Assert.AreEqual(2, DataType.ArrayDepth);
  finally
    DataType.Free;
  end;
end;
...
begin
  ...
  try
    ...
    Model.Clear;
    Assert.AreEqual(0, Model.Classes.Count);
...
begin
  Field := TGeneratorField.Create;
  try
    Assert.AreEqual(-1, Field.SourcePosition);
  finally
    Field.Free;
  end;
end;
